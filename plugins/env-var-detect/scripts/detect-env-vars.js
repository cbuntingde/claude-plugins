...
/**
 * Environment Variable Detection Script
 * Scans code for environment variable usage and detects missing definitions
 */
...
// Environment variable patterns for different languages
const patterns = {
  javascript: [
 ...
 */
function findCodeFiles(dir, excludeDirs = ['node_modules', '.git', 'dist', 'build', 'vendor', '__pycache__']) {
  const files = [];
 ...
 */
function detectMissingEnvVars(rootDir = process.cwd(), scanAll = false) {
  ...
  console.log(`${colors.yellow}${colors.bright}⚠️  Found ${uniqueMissingVars.length} missing environment variable(s):${colors.reset}\n`);
...
const scanAll = args.includes('--scan') || args.includes('-s');
const targetDir = args.find(arg => !arg.startsWith('-')) || process.cwd();

detectMissingEnvVars(targetDir, scanAll);
